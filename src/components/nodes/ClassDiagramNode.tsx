    ...
    >
      ...
      {!dragging && data.fields.length > 0 && (
        <div className="anodi-export-data mx-2 mt-2 rounded border border-gray-200 bg-gray-50 px-2 py-1 dark:border-gray-700 dark:bg-gray-800">
          {data.fields.map((f) => (
            <div key={f.id} className="relative truncate text-[10px] text-gray-700 dark:text-gray-300" style={{ height: MEMBER_H, lineHeight: `${MEMBER_H}px` }}>
              <Handle
                type="target"
      ...
      {!dragging && data.methods.length > 0 && (
        <div className="anodi-export-data mx-2 mb-2 mt-2 rounded border border-gray-200 bg-gray-50 px-2 py-1 dark:border-gray-700 dark:bg-gray-800">
          {data.methods.map((m) => (
            <div key={m.id} className="relative truncate text-[10px] text-gray-700 dark:text-gray-300" style={{ height: MEMBER_H, lineHeight: `${MEMBER_H}px` }}>
              <Handle
                type="target"
                position={Position.Left}
                id={`method-${m.id}-left`}
